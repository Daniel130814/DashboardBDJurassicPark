...
class VeterinariosView:
    ...
    def build(self):
        ctk.CTkLabel(self.parent, text="🩺  Veterinarios",
                     font=ctk.CTkFont(size=18, weight="bold")
        ).pack(pady=(16,2), anchor="w", padx=24)
        ctk.CTkLabel(self.parent,
                     text="Especialistas médicos del parque",
        ...
        for i, col in enumerate(["Matrícula","Nombre","Apellido",
        ...
        ctk.CTkLabel(frame_form, text="Registrar veterinario",
        ...
        for label, ph in [
            ...
            ("Nombre",       "Ej: Ellie"),
            ("Apellido",     "Ej: Sattler"),
            ("Teléfono",     "Ej: 555-0210"),
            ("Especialidad", "Ej: Paleobotánica"),
        ]:
    ...
    def cargar_tabla(self):
        ...
        try:
            ...
            cur  = conn.cursor()
            cur.execute("""
                SELECT MatriculaVete, nombre, apellido,
                       telefono, especialidad
                FROM   Veterinario
                ORDER  BY apellido
            """)
            for r, fila in enumerate(cur.fetchall(), 1):
                self.ids_fila.append(fila.MatriculaVete)
                vals = [str(fila.MatriculaVete), fila.nombre,
                        fila.apellido, fila.telefono, fila.especialidad]
                ws = []
        ...
        try:
            ...
            cur.execute("""
                INSERT INTO Veterinario
                    (MatriculaVete, nombre, apellido,
                     telefono, especialidad)
                VALUES (?, ?, ?, ?, ?)
            """, (int(vals["Matrícula"]), vals["Nombre"],
                  vals["Apellido"], vals["Teléfono"],
                  vals["Especialidad"]))
            conn.commit()
            conn.close()
    ...
    def eliminar(self):
        if self.seleccionado is None:
            ...
            return
        mat = self.ids_fila[self.seleccionado]
        try:
            conn = get_connection()
            cur  = conn.cursor()
            cur.execute(
                "DELETE FROM Veterinario WHERE MatriculaVete = ?", (mat,))
            conn.commit()
            conn.close()
